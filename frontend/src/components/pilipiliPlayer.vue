          ...
          <div :data-shadow-show="ctrlHidden">
            ...
            <div class="flex w-full h-[35px] justify-between leading-[22px] mt-5 px-3 transition-all duration-200 ease-out">
              ...
              <div class="flex justify-end text-white">
                ...
                <div></div>
                <!-- 全屏 -->
                <div @click="handleFullscreen" class="fill-white text-control-color w-9 h-[22px] leading-[22px] outline-0 relative text-[0px] text-center z-[2] hover:text-white box-content">
                  <div class="cursor-pointer w-full">
                    <span class="h-[22px] opacity-90 hover:opacity-100 align-middle inline-flex select-none w-full">
...
const handleTimeUpdate = (event) => {
  if(!progressPressDown.value) {
    ...
    thumbOffsetX.value = video.value.currentTime / props.options.duration * (rect.width - 20)
...
const handleProgressDown = (event) => {
  ...
  currentTime.value = offsetX / rect.width * props.options.duration
  thumbOffsetX.value = currentTime.value / props.options.duration * (rect.width - 20)
}

const progressMove = (event) => {
  const rect = progressArea.value.getBoundingClientRect()
  const offsetX = event.clientX - rect.left
  const progressRate = offsetX / rect.width
  if(progressRate < 0) {
    currentTime.value = 0
    thumbOffsetX.value = 0
...
const handleTimeSeekClick = async () => {
  ...
  await nextTick()
  prevTime.value = getVideoTime(currentTime.value)
  timeSeekInp.value.value = getVideoTime(currentTime.value)
  timeSeekInp.value.focus()
}
const handleTimeSeekBlur = (event) => {
  if(prevTime.value !== event.target.value) {
